...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/login/**","/register/**").permitAll(); // qu'on soit connecter ou pas on doit pouvoir avoir acces a ces formulaires
		  http.authorizeRequests().antMatchers(HttpMethod.POST, "/tasks/**").hasAuthority("ADMIN") ;//indique a spring security que la personne qui a le droit d'ajout doit avoir le role ADMIN
